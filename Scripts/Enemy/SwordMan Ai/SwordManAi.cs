...
public class SwordManAi : MonoBehaviour
{
    ...
    //Flipping Sprite
    private int facingDirection = 1; // Initially the enemy is facing right
    private void FlipSprite(Vector2 direction)
    {
        ...
        {
            // Determine the new facing direction
            int newFacingDirection = direction.x > 0 ? -1 : 1;

            // Only change the scale if the facing direction has changed
            if (newFacingDirection != facingDirection)
            {
                facingDirection = newFacingDirection;
                visualScale.x *= facingDirection;
